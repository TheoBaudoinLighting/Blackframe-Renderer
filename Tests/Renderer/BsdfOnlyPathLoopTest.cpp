...
template <SpectrumScalar Scalar> void expect_lambertian_environment_estimator_is_exact() {
    ...
    const auto initial = PathStateT<Scalar>::create(beta, SpectrumFor<Scalar>{}, 0, Scalar{1.5},
                                                    wavelengths, PathDeltaFlags::none, medium);
    ASSERT_TRUE(initial.has_value());

    const auto first = trace_bsdf_only(
        *ray, *initial, stream, std::span<const SurfaceFor<Scalar>>{&*surface, 1}, *environment, 1);
    const auto replay = trace_bsdf_only(
        *ray, *initial, stream, std::span<const SurfaceFor<Scalar>>{&*surface, 1}, *environment, 1);
...
template <SpectrumScalar Scalar> void expect_explicit_termination_semantics() {
    ...
    const auto miss = trace_bsdf_only(*miss_ray, *state, stream,
                                      std::span<const SurfaceFor<Scalar>>{}, *environment, 0);
    ...
    EXPECT_EQ(cutoff->termination, BsdfOnlyPathTermination::maximum_depth);
    ...
    const auto absorbed =
        trace_bsdf_only(*front_ray, *state, stream,
                        std::span<const SurfaceFor<Scalar>>{&*absorber, 1}, *environment, 2);
...
template <SpectrumScalar Scalar> void expect_invalid_paths_fail_explicitly() {
    ...
    const auto wrong_medium =
        trace_bsdf_only(*wrong_medium_ray, *state, stream,
                        std::span<const SurfaceFor<Scalar>>{&*surface, 1}, *environment, 1);
    ...
    const auto coplanar =
        trace_bsdf_only(*coplanar_ray, *state, stream,
                        std::span<const SurfaceFor<Scalar>>{&*surface, 1}, *environment, 1);
    ...
    const auto negative_state =
        PathStateT<Scalar>::create(negative_beta, black, 0, Scalar{1}, wavelengths,
    ...
    const auto negative =
        trace_bsdf_only(*valid_ray, *negative_state, stream,
                        std::span<const SurfaceFor<Scalar>>{&*surface, 1}, *environment, 1);
    ...
    const auto tiny_state = PathStateT<Scalar>::create(tiny_beta, black, 0, Scalar{1}, wavelengths,
                                                       PathDeltaFlags::none, MediumId{.value = 1});
    ASSERT_TRUE(half_surface.has_value());
    ASSERT_TRUE(tiny_state.has_value());
    const auto underflow =
        trace_bsdf_only(*valid_ray, *tiny_state, stream,
                        std::span<const SurfaceFor<Scalar>>{&*half_surface, 1}, *environment, 1);
...
TEST(BsdfOnlyPathLoopTest, DerivedHitErrorDoesNotSkipNearbyGeometry) {
    ...
    const auto traced = trace_bsdf_only(*ray, *state, stream, surfaces, *environment, 1);
    ASSERT_TRUE(traced.has_value());
    EXPECT_EQ(traced->state.accumulated_radiance(), white);
    EXPECT_EQ(traced->state.depth(), 1U);
    EXPECT_EQ(traced->termination, BsdfOnlyPathTermination::maximum_depth);
...
estimate_cornell(const std::span<const ReferenceBsdfOnlyTriangleSurface> surfaces,
    ...
    for (auto probe = std::size_t{0}; probe < probe_directions.size(); ++probe) {
        ...
        for (auto sample_index = std::uint64_t{0}; sample_index < sample_count; ++sample_index) {
            ...
            const auto traced = trace_bsdf_only(*ray, *state, stream, surfaces, *environment, 3);
            if (!traced.has_value()) {
